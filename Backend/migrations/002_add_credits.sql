...
BEGIN
    ...
    SELECT ai_credits_balance, is_premium 
    INTO current_credits, is_premium
    FROM profiles 
    WHERE id = user_id
    ...
    -- Gib Erfolg zurück
    RETURN QUERY SELECT TRUE, current_credits - 1, NULL::TEXT;
END;
$$;
...
BEGIN
    ...
    -- Gib neuen Stand zurück
    RETURN QUERY 
    SELECT TRUE, ai_credits_balance 
    FROM profiles 
    WHERE id = user_id;
END;
$$;
